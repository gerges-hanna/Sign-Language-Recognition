...
class LSTM(AbstractModel):
    ...
    def initialize_model(self):
        ...
        for i in range(self.n_hidden - 1):
            model.add(keras.layers.LSTM(self.n_neurons, return_sequences=True, activation=self.activation))
        model.add(keras.layers.LSTM(self.output_shape, return_sequences=False, activation=self.activation))
